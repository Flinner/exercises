...
subString start end str = take len $ drop start str
  where
    begin = if start < 0 then 0 else start
    len = end - begin + 1
...
lowerAndGreater n list = go 0 0 list
  where
    ...
    go nless nge l
      | null l = show n ++ " is greater than " ++ show nge ++ " elements and lower than " ++ show nless ++ " elements"
      | n > x = go nless (1 + nge) xs
      | n < x = go (1 + nless) nge xs
      | n == x = go nless nge xs
      where
        x = head l
        xs = tail l
